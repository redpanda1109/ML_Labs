# split data before that
# 1) fill mising value -> numerical to mean value and categorical to mode
# 2) convert all the features to number values
# 3) Use euclidean dist
# 4) sort dist: heap, merge, quick sort
# 5) take k nearest negihbours
# 6) tie break - by order in excel
# 7) based on majority assign class
# 8) tie breaker - the smallest dist is first, then oder in excel
...
def winning_class(class_type):
    target_class=[]  #finding the tw unique classes
    for c in class_type:
